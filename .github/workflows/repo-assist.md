...
# Do not switch engines without a maintainer decision. The provider
# definition is written to ~/.pi/agent/models.json by the "Configure pi
# custom provider" step below; the key arrives via BAI_API_KEY (repo secret,
# maintainer-provisioned, never committed). API flavor assumption:
# openai-completions (chosen from the /v1 base URL); if b.ai speaks a
# different flavor, change `api` in the step below.
engine:
  id: pi
  model: qwen3.8-flash
  # NOTE: no `env:` here on purpose. gh-aw gives the agent sandbox every
  # runner env var EXCEPT secrets referenced in engine.env (compiled to
  # awf --exclude-env), so BAI_API_KEY cannot reach pi this way, in strict
  # mode or otherwise (verified by compiling both). gh-aw issue #20416
  # confirms custom-provider auth is unsupported: secret handling is
  # coupled to built-in runtimes. The key plumbing is the missing piece;
  # everything else (model, models.json step, firewall host) is ready.
...
safe-outputs:
  # Threat detection cannot run on pi's custom provider, so it runs on the
  # gemini engine (GEMINI_API_KEY is already a repo secret). No new
  # credential needed for this path.
  threat-detection:
    engine: gemini
  messages:
